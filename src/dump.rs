...
mod tests {
    ...
    fn split_dump_separates_by_type() {
        ...
        assert_eq!(
            split.sequences,
            "CREATE SEQUENCE \"public\".\"user_id_seq\" AS integer INCREMENT BY 1 MINVALUE 1 MAXVALUE 2147483647 START WITH 1 CACHE 1;\n"
        );
        assert_eq!(
            split.tables,
            "CREATE TABLE \"public\".\"users\" (\n    \"email\" TEXT NOT NULL,\n    \"id\" BIGINT NOT NULL,\n    PRIMARY KEY (\"id\")\n);\n"
        ...
        assert_eq!(
            split.functions,
            "CREATE FUNCTION \"public\".\"get_user\"(\"user_id\" bigint) RETURNS users LANGUAGE sql VOLATILE AS $$SELECT * FROM users WHERE id = user_id;$$;\n"
        );
        assert_eq!(
            split.views,
        ...
        let sections = [
            ("extensions", split.extensions.as_str(), "CREATE EXTENSION"),
            ("types", split.types.as_str(), "CREATE TYPE"),
            ("sequences", split.sequences.as_str(), "CREATE SEQUENCE"),
            ("tables", split.tables.as_str(), "CREATE TABLE"),
            ("functions", split.functions.as_str(), "CREATE FUNCTION"),
            ("views", split.views.as_str(), "CREATE VIEW"),
            ("triggers", split.triggers.as_str(), "CREATE TRIGGER"),
            ("policies", split.policies.as_str(), "CREATE POLICY"),
            ("grants", split.grants.as_str(), "GRANT"),
        ];
        for (section_name, section_text, _) in &sections {
            for (other_name, _, other_keyword) in &sections {
                if other_name == section_name {
                    continue;
                }
                assert!(
                    !section_text.contains(other_keyword),
                    "{section_name} section unexpectedly contains {other_keyword:?}: {section_text:?}"
                );
            }
        }
    }
    ...
    fn split_dump_includes_grants() {
        ...
        assert!(
            split.tables.contains("CREATE TABLE"),
            "Tables section should contain CREATE TABLE"
        );
        assert!(
            !split.tables.contains("GRANT"),
            "Tables section should not contain the GRANT statement"
        );
        assert!(
            !split.grants.contains("CREATE TABLE"),
            "Grants section should not contain CREATE TABLE"
        );
    }
}
