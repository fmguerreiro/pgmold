...
#[test]
fn roundtrip_stable_for_overlong_autogenerated_fk_name() {
    // The inline FK auto-name `{child}_{col}_fkey` is 64 bytes here, crossing
    // the 63-byte NAMEDATALEN limit only after concatenation. `parse(sql)`
    // records it as an overlong diagnostic; `dump` emits the truncated name, so
    // `parse(dump(parse(sql)))` records nothing. The two schemas must still
    // compare equal because the diagnostic is not schema state.
    assert_parse_dump_parse_stable(
        "CREATE SCHEMA IF NOT EXISTS s;\nCREATE TABLE s.a77mn_4_2_duyio_lv4_2h_1w3x (id bigint PRIMARY KEY);\nCREATE TABLE s.z__y_6_o___6_0p0_4d_t72___7d (\n  id bigint PRIMARY KEY,\n  a77mn_4_2_duyio_lv4_2h_1w3x_id bigint REFERENCES s.a77mn_4_2_duyio_lv4_2h_1w3x(id)\n);",
    );
}
