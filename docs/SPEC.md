...
| `deny_drop_table_in_prod` | Error | DropTable when `PGMOLD_PROD=1`; the same production gate takes precedence over `--allow-destructive` for every data-destructive drop rule (column, table, partition, view, enum, trigger, sequence, unique constraint, schema, extension, domain), each named `deny_drop_<op>_in_prod`; schema-only drops (index, primary key, foreign key, check/exclusion constraint, policy, function, aggregate) are not gated |
